...
class Datasets(torch.utils.data.Dataset):
    ...
    @staticmethod
    def filter_data(data, targets, cl_filter, idxs):
    ...
    class CIFAR10():
        def __init__(self, path, download, cl_filter=None):
            ...
            self.data = torch.cat((torch.tensor(self.trainset.data), torch.tensor(self.valset.data))).movedim(-1,
                                                                                                              1) / 255
    ...
    def __init__(self, dataset_name='MNIST', mode='train', path='../data', cv=0, download=True, cl_filter=None,
                 verbose=False):
    ...
    def filter_dataset(self, cl_filter: [[], []]):
        self.train_idxs_f = Datasets.filter_indices(self.train_idxs, self.targets[self.train_idxs],
                                                    torch.tensor(cl_filter[0]))
        self.val_idxs_f = Datasets.filter_indices(self.val_idxs, self.targets[self.val_idxs],
                                                  torch.tensor(cl_filter[1]))
