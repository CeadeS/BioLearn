...
from experiment import run_sequencial, run_whole
from modules.alexnetMini import AlexNetMini
from modules.linear import FCNet
from modules.resnetMini import resnet20, resnet32, resnet44, resnet56, resnet110, resnet1202
...
def run(exp='whole', model_class="FCNet", model_args={'in_feats': 784, 'shapes': [1000], 'num_classes': 10},
        optimizer='SGD', optimizer_args={'lr': 1e-3}, fuzzy_learning_rate_factor_nu=0.09, batch_size=100,
        vbatch_size=1000, dampening_factor=0.6, fold=5, crystal_thresh=4.5e-5, rejuvenation_parameter_dre=14.0,
        weight_splitting_Gamma=2, epochs=1, dataset_name='MNIST', purge=True, scale_grad=True, crystallize=False,
        seq_num=2, verbose=False, cuda_device=None):
    ...
    if exp == "whole":
        acc, res = run_whole(model_class=model_class, model_args=model_args, optimizer=optimizer,
                             optimizer_args=optimizer_args, fuzzy_learning_rate_factor_nu=fuzzy_learning_rate_factor_nu,
                             batch_size=batch_size, vbatch_size=vbatch_size, dampening_factor=dampening_factor,
                             fold=fold, crystal_thresh=crystal_thresh,
        ...
        acc, res = run_sequencial(model_class=model_class, model_args=model_args, optimizer=optimizer,
                                  optimizer_args=optimizer_args,
                                  fuzzy_learning_rate_factor_nu=fuzzy_learning_rate_factor_nu, batch_size=batch_size,
                                  vbatch_size=vbatch_size, dampening_factor=dampening_factor, fold=fold,
                                  crystal_thresh=crystal_thresh, rejuvenation_parameter_dre=rejuvenation_parameter_dre,
                                  weight_splitting_Gamma=weight_splitting_Gamma, epochs=epochs,
                                  dataset_name=dataset_name, purge=purge, scale_grad=scale_grad,
                                  crystallize=crystallize, num_classes=num_classes, seq_num=seq_num, verbose=False,
                                  cuda_device=cuda_device)
...
kwargs = {"exp": "whole", "model_class": "resnet20",
          "model_args": {"in_feats": 784, "shapes": [1000], "num_classes": 10}, "optimizer": 'SGD',
          "optimizer_args": {"lr": 1e-3}, "fuzzy_learning_rate_factor_nu": 0.09, "batch_size": 100, "vbatch_size": 100,
          "dampening_factor": 0.6, "fold": 5, "crystal_thresh": 4.5e-5, "rejuvenation_parameter_dre": 14.0,
          "weight_splitting_Gamma": 2, "epochs": 200, "dataset_name": "MNIST", "purge": True, "scale_grad": True,
          "crystallize": False, "verbose": False, "seq_num": 2,
    ...
    for dataset_name in dataset_names:
        for accum in accums:
            for combination in combinations:
                ...
                kwargs['weight_splitting_Gamma'] = accum
                kwargs['purge'] = purge
